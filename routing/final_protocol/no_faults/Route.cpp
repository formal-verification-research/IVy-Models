...
int route(int cur_x, int cur_y, int dest_x, int dest_y, Count& num_fault, int max_dim) 
{
  ...
  filename << "s" << start_x << start_y << "_d" << dest_x << dest_y;
  ...
  Node node[100][100]; //X then Y coordinate
  ...
  while (!delivered) //routing with faulty link checks
  {
    ...
    // go east if:
    else if (cur_x != max_dim && (traveling != "west") && node[cur_x][cur_y].east_ok && (dest_x > cur_x + 1 || (dest_x > cur_x && dest_y == cur_y + 1))) {
      cur_x++;
      traveling = "east";
      ss << traveling << " to " << cur_x << cur_y << endl;
    }
    ...
    // go north if:
    else if (cur_y != max_dim && (traveling != "south") && node[cur_x][cur_y].north_ok && dest_y > cur_y) {
      cur_y++;
      traveling = "north";
      ss << traveling << " to " << cur_x << cur_y << endl;
    }
    ...
    //ELSE go south if
    else if (cur_y != 0 && cur_y >= dest_y && traveling != "north" && node[cur_x][cur_y].south_ok /**/ && (traveling != "east" || dest_x != max_dim || dest_y != max_dim /*|| node[cur_x][cur_y].east_ok*/ )) {
      cur_y--;
      traveling = "south";
      ss << traveling << " two " << cur_x << cur_y << endl;
    }
    ...
    //ELSE go east if
    else if (cur_x != max_dim && cur_x <= dest_x && (traveling != "west" || cur_x == dest_x || (cur_x + 1 == dest_x && cur_y + 1 != dest_y)) 
    ...
    //ELSE go north if
    else if (cur_y != max_dim && cur_y <= dest_y && (traveling != "south" || cur_x <= dest_x) && node[cur_x][cur_y].north_ok) {
      cur_y++;
      traveling = "north";
      ss << traveling << " two " << cur_x << cur_y << endl;
    }
