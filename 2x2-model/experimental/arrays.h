...
class arrays {
  public:
    ...
    class arr : public std::vector<int>{
        public: size_t __hash() const { return hash_space::hash<std::vector<int> >()(*this);};
    };
    bool _generating;
    long long __CARD__t;
    virtual int arr__value(const arr& a, int i);
    virtual int arr__end(const arr& a);
    arrays();
    virtual void ext__arr__set(arr& a, int x, int y);
    virtual void __init();
    virtual arr ext__arr__create(int s, int y);
    virtual arr ext__tabulate(int max);
