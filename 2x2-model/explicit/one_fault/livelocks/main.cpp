...
int main() 
{
  ...
  while(getline(livelist, line)) {
    ...
    for (int c = 0; c < 17; c++) {
      stringstream rt;
      rt << pattern[c + 1]; //buffer of 1, to rule out extraneous incoming directions
    ...
    ofile << in1.rdbuf();
    ofile << "    node(" << faulty_x_1 << "," << faulty_y_1 << ")." << longDirection(faulty_dir_1) << "_free := false;" << endl;
    ofile << "    node(" << faulty_x_2 << "," << faulty_y_2 << ")." << longDirection(faulty_dir_2) << "_free := false;" << endl;
    ...
    for (int d = 0; d < loopSize; d++) {
      int calc = ai[0] + d - 1;
      ofile << "invariant node(" << to_x[calc] << "," << to_y[calc] << ").has_flit -> flit.traveling = " << dir[calc] << endl;
    }
